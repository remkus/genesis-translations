 ...
 * Based on i18n module by Yoast ... https://github.com/Yoast/i18n-module
 */
class ThemeMix_Genesis_Translations_i18n {
	...
	private function hide_promo() {
		$hide_promo = get_transient( 'thememix_genesis_translations_i18n_' . $this->project_slug . '_promo_hide' );
		if ( ! $hide_promo ) {
			if ( filter_input( INPUT_GET, 'remove_i18n_promo', FILTER_VALIDATE_INT ) === 1 ) {
				// No expiration time, so this would normally not expire, but it wouldn't be copied to other sites etc.
				set_transient( 'thememix_genesis_translations_i18n_' . $this->project_slug . '_promo_hide', true );
				$hide_promo = true;
			}
		}
	...
	private function promo_message() {
		...
		if ( $this->translation_exists && $this->translation_loaded && $this->percent_translated < 90 ) {
			$message = __( 'As you can see, there is a translation of this plugin in %1$s. This translation is currently %3$d%% complete. We need your help to make it complete and to fix any errors. If you would like to help, please visit %4$s!', $this->textdomain );
		} else if ( ! $this->translation_loaded && $this->translation_exists ) {
			$message = __( 'You\'re using WordPress in %1$s. While %2$s has been translated to %1$s for %3$d%%, it\'s not been shipped with the plugin yet. You can help! Visit %4$s to help complete the translation!', $this->textdomain );
		} else if ( ! $this->translation_exists ) {
			$message = __( 'You\'re using WordPress in a language we don\'t support yet. We\'d love for %2$s to be translated in that language too, but unfortunately, it isn\'t right now. You can change that! Visit %4$s to help translate it!', $this->textdomain );
		}
	...
	private function find_or_initialize_translation_details() {
		$set = get_transient( 'thememix_genesis_translations_i18n_' . $this->project_slug . '_' . $this->locale );

		if ( ! $set ) {
			$set = $this->retrieve_translation_details();
			set_transient( 'thememix_genesis_translations_i18n_' . $this->project_slug . '_' . $this->locale, $set, DAY_IN_SECONDS );
		}
	...
	/**
	 * Retrieve the translation details from WordPress.org
	 *
	 * @access private
	 *
	 * @return object|null
	...
	private function retrieve_translation_details() {
		$api_url = trailingslashit( $this->glotpress_url ) . 'api/projects/wp-plugins/' . $this->project_slug;
	...
	/**
	 * Set the needed private variables based on the results from WordPress.org
	 *
