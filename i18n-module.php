...
class ThemeMix_Genesis_Translations_i18n {
	...
	private function promo_message() {
		...
		if ( $this->translation_exists && $this->translation_loaded && $this->percent_translated < 90 ) {
			$message = __( 'As you can see, there is a translation of this plugin in %1$s. This translation is currently %3$d%% complete. We need your help to make it complete and to fix any errors. If you would like to help, please visit %4$s!', 'genesis-translations' );
			...
			$message = __( 'You\'re using WordPress in a language we don\'t support yet. We\'d love for %2$s to be translated in that language too, but unfortunately, it isn\'t right now. You can change that! Visit %4$s to help translate it!', 'genesis-translations' );
		}
	...
	public function promo() {
		...
		if ( $message ) {
			...
			echo '<h2>' . sprintf( __( 'Translation of %s', 'genesis-translations' ), $this->plugin_name ) . '</h2>';
	...
	private function translation_details() {
		...
		$this->translation_loaded = is_textdomain_loaded( 'genesis-translations' );
